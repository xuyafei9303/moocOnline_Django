...
class OrgView(View):
    ...
    def get(self, request):
        ...
        if category:
            all_orgs = all_orgs.filter(categray=category)
...
class TeacherDetailView(View):
    def get(self, request, teacher_id):
        ...
        teacher.save()
        all_courese = Course.objects.filter(teacher=teacher)

        has_teacher_faved = False
        if UserFavorate.objects.filter(user=request.user, fav_type=3, fav_id=teacher.id):
            has_teacher_faved = True

        has_org_faved = False
        if UserFavorate.objects.filter(user=request.user, fav_type=2, fav_id=teacher.org.id):
            has_org_faved = True
